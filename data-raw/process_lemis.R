...
save_object("cleaned_data/lemis_2000_2013_cleaned.csv", bucket = "eha.wild.db",
            file = h("data-raw", "lemis_2000_2013_cleaned.csv"))
save_object("csv_by_year/lemis_2000_2013.csv", bucket = "eha.wild.db",
...
lemis_raw <- read_csv(h("data-raw", "lemis_2000_2013_cleaned.csv"),
                      col_types =cols(
...
lemis <- lemis_raw %>%
  select(control_number, species_code, taxa, genus, species, subspecies,
  ...
  mutate(quantity = as.integer(as.numeric(quantity)),
